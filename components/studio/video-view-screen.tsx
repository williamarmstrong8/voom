...
import { ArrowLeft, Check, Download, Expand, Link2, Loader2, Pause, Pencil, Play, Volume2, VolumeX, Video as VideoIcon } from "lucide-react"
...
export function VideoViewScreen({
  ...
  /** "app" shows library/edit chrome; "share" is the public read-only view. */
  variant?: "app" | "share"
}) {
  ...
  const copyShareLink = async () => {
    try {
      ...
      console.error("[v0] copy share link failed:", error)
    }
  }
  ...
  const guideSteps = video.kind === "project" ? video.editor_state?.guideSteps ?? [] : []
  ...
  return (
    <main
      className={cn(
        "flex w-full flex-col gap-4 px-5 py-8 lg:px-8",
        isShare ? "min-h-svh" : "min-h-[calc(100svh-3rem)]",
      )}
    >
      ...
      >
        {playbackUrl ? (
          <div
            className={cn(
              "relative overflow-hidden rounded-lg border border-border bg-black shadow-sm",
              hasGuide ? "w-full min-w-0 lg:flex-1 lg:max-w-5xl" : "w-full max-w-7xl",
            )}
